...
def generate_pdf_report(html_content: str) -> bytes:
    """Converts HTML report string to native PDF binary bytes using xhtml2pdf or ReportLab."""
    # 1. Primary Engine: xhtml2pdf (HTML -> PDF)
    try:
        from xhtml2pdf import pisa
        pdf_buffer = io.BytesIO()
        pisa_status = pisa.CreatePDF(html_content, dest=pdf_buffer)
        if not pisa_status.err and len(pdf_buffer.getvalue()) > 100:
            return pdf_buffer.getvalue()
    except Exception as e:
        print(f"[Warning] xhtml2pdf conversion notice: {e}")
    ...
    # 3. Fallback Engine: ReportLab Native PDF Builder
    try:
        ...
        c.drawString(50, 750, "DataCleanAI Executive Quality Summary Report")
        c.setFont("Helvetica", 10)
        c.drawString(50, 730, "Generated by DataCleanAI Automated Quality Engine")
